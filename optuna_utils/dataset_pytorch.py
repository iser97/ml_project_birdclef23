...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='test':  
            ...
            sig = sig.unsqueeze(0)
            sig = audio_augmentation(sig, CFG.sample_rate, num_augmented_sampels=1, num_samples=self.input_length)
            sig = sig.squeeze()
            padding_mask = torch.zeros(1, sig.shape[0]).bool().squeeze(0)
            if self.transform:
                sig = self.transform(sig)
            return sig, padding_mask, label
...
class MusicnnDataset(AudioDataset):
    ...
    def __getitem__(self, idx):
        ...
        mfcc_stack = torchaudio.transforms.MelSpectrogram(
            ...
            f_min=CFG.fmin, 
            f_max=CFG.fmax, 
            n_mels=CFG.img_size[0])(mfcc_stack)
        mfcc_stack = mfcc_stack.repeat(3, 1, 1)
        ...
        if self.mode=='train' or self.mode=='eval':
            ...
            data = mfcc_stack[:, :, random_idx:random_idx+self.input_length]
        else:
    ...
    def mfcc_expand(mfcc, input_length):
        ...
        for i in range(factor):
            if type(mfcc)==np.ndarray:
                start = np.concatenate([start, mfcc], axis=2)
            elif type(mfcc)==torch.Tensor:
                start = torch.cat([start, mfcc], dim=2)
